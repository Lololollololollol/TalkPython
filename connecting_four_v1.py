...
def choose_location(board, symbol):
    ...
    if column < 0 or column >= len(board[0]):
        ...
        # We have a 'column' and want to decide the lowest row of the board where a new player symbol can be put
        # 1) Check entries from bottom to top whether they are None or have a symbol
        # 2) If there is a None cell, set it to the player's symbol, then break
        # (3) If there is no None cell, give an error to the user and ask for another column)
...
def announce_turn(player):
    print()
    print(f"It's {player}'s turn. Please select a column from the board.")
    print()
